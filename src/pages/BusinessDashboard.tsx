...
import {
  ref as storageRef, deleteObject, listAll, uploadBytes, getDownloadURL,
} from "firebase/storage";
import {
  signOut, deleteUser, reauthenticateWithCredential, EmailAuthProvider, reauthenticateWithPopup, GoogleAuthProvider,
} from "firebase/auth";
...
const BusinessDashboard = () => {
  ...
  const [editedService, setEditedService] = useState<Service>({
    name: "", price: 0, duration: 0, pricePlus: false,
  });
    ...
    const fetchBusiness = async () => {
      ...
      if (snap.exists()) {
        ...
        const svcs: Service[] = (data.services || []).map((s: any) => ({
          name: s.name, price: s.price, duration: s.duration, pricePlus: !!s.pricePlus,
        }));
        setServices(svcs);
      ...
      try {
        ...
        if (err?.code === "auth/requires-recent-login") {
          ...
          if (providerId === "password") {
            ...
            const password =
              window.prompt("Re-enter your password to delete your account:") || "";
  ...
  const handleCompleteAppointment = async (id: string) => {
    try {
      await setDoc(
        ...
        { status: "completed", active: false, completedAt: new Date().toISOString() },
        { merge: true }
      );
                      ...
                      {editingServiceIndex === idx ? (
                        <>
                          ...
                          <input
                            ...
                            onChange={(e) => setEditedService({ ...editedService, duration: parseInt(e.target.value || "0", 10) })}
                            className="border p-1 rounded w-1/4"
                          ...
                          <label className="inline-flex items-center gap-1 w-1/4">
                            <input
                              ...
                              onChange={(e) => setEditedService({ ...editedService, pricePlus: e.target.checked })}
                            />
                            <span className="text-xs">Show as “starting at” (+)</span>
                          </label>

                          <button className="text-green-600 text-xs font-medium" onClick={() => {
                          ...
                          <button className="text-gray-500 text-xs font-medium" onClick={() => setEditingServiceIndex(null)}>
                            Cancel
                          </button>
                        </>
                      ) : (
                        <>
                          ...
                          <span className="w-1/4">${service.price.toFixed(2)}{service.pricePlus ? "+" : ""}</span>
                          <span className="w-1/4">{service.duration} mins</span>
                          <button className="text-blue-600 text-xs font-medium" onClick={() => { setEditingServiceIndex(idx); setEditedService(service); }}>
                            Edit
                          </button>
                          <button className="text-red-600 text-xs font-medium" onClick={() => {
